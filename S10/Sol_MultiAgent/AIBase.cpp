...
void AAIBase::UpdateIfElseLogic(float DeltaTime)
{
    ...
    else if (m_isRetreating)
    {
        ...
        SetNewMoveDestination(m_retreatDestination);
    }
    else if (m_isInvestigatingExplosion)
    {
        ...
        SetNewMoveDestination(m_lastExplosionPos);

        //Has the agent reached the investigation point or saw the a target in the process    
        if ((actorLocation - m_lastExplosionPos).Size2D() < 50.f)
        {
            m_isInvestigatingExplosion = false;
            GoToNextPatrol();
        }
    ...
    {
        //Ask the group for an updated lkp
        bool targetFound = false;
        m_currentTargetLkpInfo = AiAgentGroupManager::GetInstance()->GetLKPFromGroup(targetPlayer->GetActorLabel(), targetFound);

        TargetLKPInfo::ELKPState currentInvestigatedLKPState = m_currentTargetLkpInfo.GetLKPState();
        currentInvestigatedLKPState = m_currentTargetLkpInfo.GetLKPState();
        ...
        if ((actorLocation - lkpPos).Size2D() < 50.f)
        {
            ...
            m_currentTargetLkpInfo.SetLastUpdatedTimeStamp(currentElapsedTime);
            GoToNextPatrol();
        }
    ...
    else if (m_CurrentDestination == FVector::ZeroVector)
    {
        ...
        else
        {
            ...
            TargetLKPInfo targetInfoFromGroup = AiAgentGroupManager::GetInstance()->GetLKPFromGroup(targetPlayer->GetActorLabel(), targetFoundByGroup);
            //Check if group knows where player might be  [Exer on LKP sharing week 11]
            if (targetFoundByGroup)
            {
                m_currentTargetLkpInfo = targetInfoFromGroup;
                m_isInvestigatingLKP = true;
            }
            else
            {
...
void AAIBase::ProcessReaction(ReactionEvent* reactionEvent)
{
    ...
    else if (reactionEvent->m_ReactionType == ReactionType_Explosion)
    {
        ...
        if (reactionEvent->m_ReactionLOS == ReactionLOS_Visual && m_IsPlayerDetected)
        {
            ProcessSightedBomb(m_bombDropPos);
        }
        else if (reactionEvent->m_ReactionLOS == ReactionLOS_Sound)
        {
            ProcessHeardBomb(m_bombDropPos);
            //Bomb has exploded agent doesn't need to Retreat anymore
            m_isRetreating = false;
        }
...
{
    //Don't process an seen bomb if you are already retreating or if you are not chasing a player
    if (m_isRetreating)
    {
        return;
    }
...
void AAIBase::ProcessHeardBomb(const FVector& bombPos)
{
    ...
    //Don't process heard bomb if you are already retreating from it or that you have detected a player
    if (m_isRetreating || m_IsPlayerDetected)
    {
        return;
    }

    //Agents needs to stop investigating a lkp
    m_isInvestigatingLKP = false;
